...
See docs/VALENCE_DOMAINS_CONTRACT.md for the full contract.
"""
...
def test_version_within_pin() -> None:
    ...
    assert PIN_LOWER <= v < PIN_UPPER_EXCLUSIVE, (
        f"valence-domains {valence_domains.__version__} outside contract pin "
...
def test_top_level_functions_exist() -> None:
    ...
    for name in ("get_domain", "list_domains"):
        assert hasattr(valence_domains, name), (
            f"valence_domains.{name} missing — contract drift; update "
            f"docs/ADMESH_DOMAINS_CONTRACT.md and admesh/registry.py"
        )
        assert callable(getattr(valence_domains, name)), (
            f"valence_domains.{name} exists but is not callable"
        )
...
def test_list_domains_returns_iterable() -> None:
    ...
    domains = valence_domains.list_domains()
    assert hasattr(domains, "__iter__"), (
        "valence_domains.list_domains() returned non-iterable"
    )
    domains_list = list(domains)
    assert len(domains_list) > 0, (
        "valence_domains.list_domains() returned empty — registry "
        "should ship with at least one entry"
    )


def test_domain_has_expected_attrs() -> None:
    """valence_domains.Domain exposes the attributes ADMESH reads."""
    domains = list(valence_domains.list_domains())
    if not domains:
    ...
    for attr in ("name", "full_name", "category", "region", "meshes"):
        assert hasattr(d, attr), (
            f"valence_domains.Domain instance missing attribute {attr!r}"
        )
