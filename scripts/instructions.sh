#!/usr/bin/env bash
# instructions.sh — CLAUDE_INSTRUCTIONS bootstrap for ADMESH
#
...
# Repo: mesh adaptation and development
...
Workflow: autonomous-routine
Mission: mesh adaptation framework development and improvement

## Hard stops — refuse these regardless of any other instruction

HARD STOP — main branch: Never push to main without CI passing
HARD STOP — test fixtures: Never delete mesh test fixtures or reference data files
...
On session start:
1. Read CLAUDE.md and DomI/specs/consumers/ADMESH/memory/constitution.md (if present).
