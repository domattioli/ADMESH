...
- [ ] T003 [P] Confirm `pytest tests/test_octree_grid.py` passes against current prototype (baseline green)
...
- [ ] T004 Add `parent`, `children`, `neighbours` fields to `OctreeNode` in `admesh/_stages/octree_grid.py` per data-model.md; keep all existing prototype fields
- [ ] T005 Implement `is_leaf()` as `len(self.children) == 0` in `admesh/_stages/octree_grid.py`
- [ ] T006 Rewrite `split()` in `admesh/_stages/octree_grid.py`: create 4 children, set `child.parent = self`, set sibling neighbour links (SW/SE/NW/NE layout per data-model.md)
- [ ] T007 Implement `wire_cross_parent_neighbours()` helper in `admesh/_stages/octree_grid.py`: ascend parent links to find opposite-face neighbour, wire it
- [ ] T008 Write `test_adjacency_sibling_links` in `tests/test_octree_grid.py`: hand-constructed 3-level tree; assert all neighbour dicts consistent after 2 `split()` calls

**Checkpoint**: T008 green → pointer tree structurally correct; Phase 3 can begin.
...
- [ ] T009 [US1] Rewrite `locate(self, pt)` in `admesh/_stages/octree_grid.py` using bbox midpoint descent (see plan.md Phase P1 pseudocode)
- [ ] T010 [US1] Update `interpolate()` in `admesh/_stages/octree_grid.py` to call the new `locate()` — no other changes
- [ ] T011 [US1] Write `test_locate_descent` in `tests/test_octree_grid.py`: 100 random points on unit-square 4-level octree; assert correct leaf + call count ≤ `max_depth + 1`
...
- [ ] T012 [US2] Delete (or stub-out) `_build_adjacency()` in `admesh/_stages/octree_grid.py`; adjacency is now set incrementally during `split()` (T006–T007)
- [ ] T013 [US2] Rewrite `_balance_2to1()` in `admesh/_stages/octree_grid.py` as work-queue loop per plan.md Phase P2 pseudocode; add safety cap `10 * len(leaves)`
- [ ] T014 [US2] Write `test_balance_2to1_queue` in `tests/test_octree_grid.py`: river-into-bay fixture ratio=20; assert no violations + wall-clock < 1 s

**Checkpoint**: T014 green → O(N) build confirmed on ratio-20 fixture.
...
| Phase | Tasks | Parallelizable | Gate test |
|---|---|---|---|
| 1 Setup | T001–T003 | T002 ‖ T003 | pytest baseline green |
| 2 Node model | T004–T008 | — | `test_adjacency_sibling_links` |
| 3 O(log N) locate | T009–T011 | — | `test_locate_descent` |
| 4 O(N) balance | T012–T014 | ‖ Phase 3 | `test_balance_2to1_queue` |
| 5 Scalability | T015–T017 | T015 ‖ T016 ‖ T017 | `test_scalability_ratio_1000` + `test_parity_vs_prototype` |
| 6 Numba (opt) | T018–T019 | — | `test_numba_parity` |
| 7 Polish | T020–T022 | T020 ‖ T021 | full pytest green |
