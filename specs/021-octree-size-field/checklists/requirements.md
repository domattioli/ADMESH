...
- A `/speckit-clarify` pass (2026-05-28) resolved four points: the elements-per-feature definition (target ≤ ¼ width AND output-mesh verification), the resolution floor (`h_min`, no separate max-depth cap), the uniform grid's status (octree default with automatic fallback), and the validation benchmark (synthetic basin+inlet fixture + a real ADCIRC multi-scale mesh). Spec updated accordingly; ready for `/speckit-plan`.
