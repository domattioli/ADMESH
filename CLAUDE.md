...
- `021-octree-size-field` — **active plan** ([plan.md](specs/021-octree-size-field/plan.md)) — replace the uniform cartesian background grid with an octree/quadtree substrate for the size function + medial axis (multi-scale medial-axis robustness; ≥ 4 elements per feature). **Modifies locked stages** `background_grid` / `medial_axis` / `mesh_size` under explicit user authorization; gated on a Constitution v2.0.0 amendment carving these stages out of Principle I.

**Constitution Principle I** still binds: 13 faithful-port stage modules MUST stay numerically identical to MATLAB. New behaviour goes in additive-layer modules (`api.py`, `fort14.py`, `boundary_types.py`, `loaders.py`, `size_field.py`, `viz.py`, `quad_prep.py`, `registry.py`, `domains.py`) — strictly additive, never replacing locked modules. **Exception (pending v2.0.0 amendment):** spec `021-octree-size-field` is authorized to modify `background_grid` / `medial_axis` / `mesh_size` for the octree substrate; the uniform faithful-port path is retained as the fallback. See its plan + the forthcoming Constitution amendment.
<!-- SPECKIT END -->
